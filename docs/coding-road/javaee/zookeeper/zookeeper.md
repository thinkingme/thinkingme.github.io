...
> zookeeper为什么采用过半选举:
> 
> 防止由脑裂造成的多个领导者。(脑裂：比如如果有多个机房)
> ❞

        在分布式系统中，脑裂是指由于网络分区等原因导致集群中的节点被划分为两个或多个互不通信的子集。在这种情况下，如果每个子集都试图选举自己的领导者，将导致系统出现多个领导者，从而破坏一致性。过半选举策略可以防止这种情况的发生，因为只有获得过半节点支持的候选者才能成为领导者。在脑裂情况下，不可能有**多个**子集包含过半节点，因此不会选举出多个领导者。
